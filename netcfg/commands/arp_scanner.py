...
# def ensure_npcap():
#     if not conf.use_pcap:
#         print("Npcap is required for ARP scanning.")
#         print("Opening download page...")
#         webbrowser.open("https://npcap.com/#download")
#         sys.exit(1)


# def arp_scan(network_cidr="192.168.1.0/24", timeout=2):
#     # craft broadcast Ethernet frame with ARP request
#     pkt = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(pdst=network_cidr)
#     answered, _ = srp(pkt, timeout=timeout)   # srp: send and receive on layer 2
#     results = []
#     for snd, rcv in answered:
#         results.append({"ip": rcv.psrc, "mac": rcv.hwsrc})
#     return results
...
# if __name__ == "__main__":
#     net = "192.168.1.0/24"  # change to your subnet
#     hosts = arp_scan(net)
#     for h in hosts:
#         print(f"{h['ip']:16}    {h['mac']}")